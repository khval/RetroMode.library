	...
	cust_fread_t cust_fwrite )
{
	...
	for (image =0 ; image < sprite->number_of_frames ; image++ )
	{
		...
		if (planar16bRow) libBase -> IExec -> FreeVec(planar16bRow);
		planar16bRow = (unsigned short *) libBase -> IExec -> AllocVecTags(  frame -> planarXSize * 2, AVT_Type, MEMF_SHARED, AVT_ClearWithValue, 0, TAG_END );
