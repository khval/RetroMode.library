
/* This file was generated by idltool 53.24. Do not edit */

#include "retromode_vectors.h"

STATIC uint32 VARARGS68K _retromode_UNIMPLEMENTED(struct RetroModeIFace *iface UNUSED)
{
  return 0;
}
STATIC CONST APTR main_v1_vectors[] =
{
    _retromode_Obtain,
    _retromode_Release,
    _retromode_UNIMPLEMENTED, /* Expunge */
    _retromode_UNIMPLEMENTED, /* Clone */
    _retromode_retroAllocEngine,
    _retromode_retroAllocVideo,
    _retromode_retroFreeVideo,
    _retromode_retroFreeEngine,
    _retromode_retroClearVideo,
    _retromode_retroDrawVideo,
    _retromode_retroDmaVideo,
    _retromode_retroOpenScreen,
    _retromode_retroCloseScreen,
    _retromode_retroApplyScreen,
    _retromode_retroPixel,
    _retromode_retroPoint,
    _retromode_retroFlash,
    _retromode_retroCycleColorsUp,
    _retromode_retroCycleColorsDown,
    _retromode_retroBAR,
    _retromode_retroOrBAR,
    _retromode_retroXorBAR,
    _retromode_retroBarPattern,
    _retromode_retroBox,
    _retromode_retroOrBox,
    _retromode_retroXorBox,
    _retromode_retroShift,
    _retromode_retroCircleFilled,
    _retromode_retroEllipse,
    _retromode_retroOrCircleFilled,
    _retromode_retroXorCircleFilled,
    _retromode_retroScreenColor,
    _retromode_retroScreenBlit,
    _retromode_retroOrScreenBlit,
    _retromode_retroXorScreenBlit,
    _retromode_retroBitmapBlit,
    _retromode_retroOrBitmapBlit,
    _retromode_retroXorBitmapBlit,
    _retromode_retroAndClear,
    _retromode_retroModeBadVideoSync,
    _retromode_AfterEffectScanline,
    _retromode_AfterEffectAdjustRGB,
    _retromode_retroTriangle,
    _retromode_retroOrTriangle,
    _retromode_retroXorTriangle,
    _retromode_retroLine,
    _retromode_retroOrLine,
    _retromode_retroXorLine,
    _retromode_retroAllocFrame,
    _retromode_retroFreeFrame,
    _retromode_retroDrawFrame,
    _retromode_retroDeleteFlash,
    _retromode_retroScreenOffset,
    _retromode_retroScreenDetach,
    _retromode_retroScreenToFront,
    _retromode_retroScreenToBack,
    _retromode_retroStar,
    _retromode_retroOrStar,
    _retromode_retroPolyLine,
    _retromode_retroPolyGon,
    _retromode_retroBoing,
    _retromode_retroLoadABKSprite,
    _retromode_retroFreeSprite,
    _retromode_retroPasteSprite,
    _retromode_retroFill,
    _retromode_retroOrFill,
    _retromode_retroBarRounded,
    _retromode_retroOrBarRounded,
    _retromode_retroXorBarRounded,
    _retromode_retroThickLine,
    _retromode_retroZoom,
    _retromode_retroBobble,
    _retromode_retroRainbowColorRange,
    _retromode_retroRain,
    _retromode_retroScreenClone,
    _retromode_retroSetRainbow,
    _retromode_retroRainbow,
    _retromode_retroAllocSpriteObjects,
    _retromode_retroFreeSpriteObjects,
    _retromode_AfterEffectDrawSrpites,
    _retromode_retroSprite,
    _retromode_retroColorRange,
    _retromode_retroPolyGonArray,
    _retromode_retroCycleOff,
    _retromode_retroFadeScreen,
    _retromode_retroCircle,
    _retromode_retroAllocDoubleBuffer,
    _retromode_retroLoadSprite,
    _retromode_retroGetSprite,
    _retromode_retroFreeSpriteObject,
    _retromode_retroScreenToBitmap,
    _retromode_retroAllocBlock,
    _retromode_retroFreeBlock,
    _retromode_retroGetBlock,
    _retromode_retroPutBlock,
    _retromode_retroPasteIcon,
    _retromode_retroMakeMask,
    _retromode_retroFreeMask,
    _retromode_retroDrawShortPlanar,
    _retromode_retroDrawMask,
    _retromode_retroGetMaxPlains,
    _retromode_retroRowToPlanar,
    _retromode_retroSaveSprite,
    _retromode_retroPasteSpriteObject,
    _retromode_retroScroll,
    (APTR)-1
};
